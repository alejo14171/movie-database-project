...
class PopularityStrategy(ClassificationStrategy):
    def classify(self, db):
        return sorted(db.get_all_movies(), key=lambda m: m.calculate_popularity(), reverse=True)
...
class ActorStrategy(ClassificationStrategy):
    ...
    def classify(self, db):
        return [movie for movie in db.get_all_movies() if self.actor_name in movie.actors]
...
class SimilarMoviesStrategy(ClassificationStrategy):
    ...
    def classify(self, db):
        target_movie = next((m for m in db.get_all_movies() if m.title == self.movie_title), None)
        if not target_movie:
            return []
        ...
        def similarity_score(movie):
            ...
            rating_diff = abs(statistics.mean(movie.ratings) - statistics.mean(target_movie.ratings))
